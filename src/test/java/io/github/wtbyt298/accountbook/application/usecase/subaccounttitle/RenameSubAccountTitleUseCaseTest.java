...
class RenameSubAccountTitleUseCaseTest {
	...
	void 補助科目IDを指定して補助科目名を変更できる() {
		...
		when(subAccountTitleRepository.findCollectionByParentId(any(), any())).thenReturn(TestObjectCreator.prepareTestObject());
		...
		assertEquals("1：ゆうちょ銀行", captured.find(changed).toString());
		assertNotEquals("1：三井住友銀行", captured.find(changed).toString());
	}
