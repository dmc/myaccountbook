...
class SubAccountTitleNameTest {
	...
	void 長さ32以内の文字列で初期化できる() {
		...
		assertDoesNotThrow(() -> SubAccountTitleName.valueOf(within));
	}
	
	@Test
	void 空文字列で初期化すると例外発生() {
		assertThrows(IllegalArgumentException.class, () -> SubAccountTitleName.valueOf(""));
	}
	
	@Test
	void 長さ32より大きい文字列で初期化すると例外発生() {
		String over = new StringWriter() {{for (int i = 0; i < 33; i++) write("A");}}.toString(); //"A"を33回連結した文字列
		assertThrows(IllegalArgumentException.class, () -> SubAccountTitleName.valueOf(over));
	}
