...
class UserPasswordTest {
	...
	void 既定のフォーマットを満たした文字列で初期化できる() {
		...
		//when
		UserPassword userPassword = UserPassword.valueOf("Test0123OK");
		
		//then
		assertEquals("Test0123OK", userPassword.value);
	}
	...
	void 英字のみで初期化すると例外発生() {
		...
		//when
		Exception exception = assertThrows(IllegalArgumentException.class, () -> UserPassword.valueOf(rawPassword));
	...
	void ハッシュ化前後のパスワードを照合すると合致する() {
		...
		//when
		UserPassword userPassword = UserPassword.valueOf(rawPassword);
		
		//then
		BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
		assertTrue(encoder.matches(rawPassword, userPassword.encode()));
	}
